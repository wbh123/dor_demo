...
public class SelectionPolicyService {
    ...
    public SelectionPolicyService(NamedParameterJdbcTemplate jdbc, StudentPreferenceService preferenceService,
        ...
        this.jdbc = jdbc; this.preferenceService = preferenceService; this.featureAccessService = featureAccessService; this.auditService = auditService;
    }
    ...
    @Transactional
    public Map<String, Object> update(boolean allowWithoutQuestionnaire, boolean allowStudentReselect,
        ...
        if (reason == null || reason.isBlank()) throw new BusinessException("POLICY_REASON_REQUIRED", "请填写策略修改原因");
        ...
        auditService.success(operator, "SELECTION_POLICY_UPDATE", "SYSTEM_SETTING", null, reason.trim(), before, after);
        return after;
    }
    ...
    public Map<String, Object> readiness(long batchId, long studentId) {
        ...
        boolean bypass = enabled(QUESTIONNAIRE_BYPASS) && featureAccessService.has(FeatureCodes.P2_QUESTIONNAIRE_BYPASS_CONTROL);
        Map<String, Object> result = new LinkedHashMap<>();
        ...
        result.put("message", completed ? "个人偏好已完成" : bypass ? "你尚未填写偏好，继续选寝将降低推荐准确性" : "请先填写个人偏好后再选寝");
        return result;
    }
    ...
    public boolean canStudentReselect() {
        return enabled(STUDENT_RESELECT) && featureAccessService.has(FeatureCodes.P2_STUDENT_RESELECT_CONTROL);
    }

    @Transactional
    public Map<String, Object> cancelAssignment(long batchId, long studentId, CurrentUser user) {
        if (!canStudentReselect()) throw new BusinessException("STUDENT_RESELECT_DISABLED", "管理员当前未开放自主取消重选", HttpStatus.FORBIDDEN);
        Integer open = jdbc.queryForObject("SELECT COUNT(*) FROM selection_batch WHERE id=:batchId AND batch_status='OPEN'",
                Map.of("batchId", batchId), Integer.class);
        if (open == null || open == 0) throw new BusinessException("BATCH_NOT_OPEN", "只有开放中的批次可以取消重选", HttpStatus.CONFLICT);
        int bed = jdbc.update("""
        ...
        int room = jdbc.update("""
                ...
                WHERE batch_id=:batchId AND student_id=:studentId AND assignment_status='ACTIVE'
                """, Map.of("batchId", batchId, "studentId", studentId));
        if (bed + room == 0) throw new BusinessException("ASSIGNMENT_NOT_FOUND", "当前批次没有可取消的选寝结果", HttpStatus.NOT_FOUND);
        auditService.success(user, "STUDENT_ASSIGNMENT_CANCEL", "SELECTION_BATCH", batchId, "学生自主取消并重选", null,
                Map.of("bedAssignments", bed, "roomAssignments", room));
        return Map.of("cancelled", true, "batchId", batchId);
    }

    private boolean hasBatchFeature(long batchId, long studentId) {
        Integer count = jdbc.queryForObject("SELECT COUNT(*) FROM student_feature WHERE batch_id=:batchId AND student_id=:studentId",
                Map.of("batchId", batchId, "studentId", studentId), Integer.class);
        return count != null && count > 0;
    }
    private int policyVersion() { Integer version = jdbc.queryForObject("SELECT COALESCE(MAX(version),0) FROM system_setting WHERE setting_key IN (:keys)", Map.of("keys", List.of(QUESTIONNAIRE_BYPASS, STUDENT_RESELECT)), Integer.class); return version == null ? 0 : version; }
    private boolean enabled(String key) { ensure(); List<String> rows=jdbc.query("SELECT setting_value FROM system_setting WHERE setting_key=:key",Map.of("key",key),(rs,n)->rs.getString(1)); return !rows.isEmpty() && Boolean.parseBoolean(rows.getFirst()); }
    private void write(String key, boolean value) { jdbc.update("UPDATE system_setting SET setting_value=:value,version=version+1 WHERE setting_key=:key",Map.of("key",key,"value",Boolean.toString(value))); }
    private void ensure() { for (String key : List.of(QUESTIONNAIRE_BYPASS, STUDENT_RESELECT)) jdbc.update("INSERT INTO system_setting(setting_key,setting_value,version) VALUES (:key,'false',0) ON DUPLICATE KEY UPDATE setting_key=VALUES(setting_key)",Map.of("key",key)); }
}
