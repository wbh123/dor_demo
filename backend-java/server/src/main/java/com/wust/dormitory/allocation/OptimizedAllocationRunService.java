...
public class OptimizedAllocationRunService {
    ...
    static void validateCandidateSet(List<Candidate> candidates) {
        ...
        for (Candidate candidate : candidates) {
            if (!students.add(candidate.studentId())) throw new BusinessException("ALLOCATION_DUPLICATE_STUDENT", "候选方案中同一学生出现多次", HttpStatus.CONFLICT);
            if (!beds.add(candidate.bedId())) throw new BusinessException("ALLOCATION_DUPLICATE_BED", "候选方案中同一床位被重复分配", HttpStatus.CONFLICT);
        }
    }
    ...
    static void requireCompleteTeams(List<Candidate> candidates, Map<Long, Integer> expectedTeamSizes) {
        ...
        for (Map.Entry<Long, Integer> entry : expectedTeamSizes.entrySet()) {
            ...
            if (members.size() != entry.getValue() || members.stream().map(Candidate::roomId).distinct().count() != 1) {
    ...
    private void insertCandidates(long runId, List<Candidate> candidates) {
        for (int start = 0; start < candidates.size(); start += CANDIDATE_INSERT_BATCH_SIZE) {
            ...
            List<Map<String, Object>> rows = candidates.subList(start, end).stream().map(this::candidateInsertRow).toList();
