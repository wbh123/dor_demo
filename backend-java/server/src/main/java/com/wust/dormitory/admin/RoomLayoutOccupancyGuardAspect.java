...
public class RoomLayoutOccupancyGuardAspect {
    ...
    public void verifyTypeChanges(
        ...
        Map<String, Object> layout = roomLayoutService.getLayout(roomId);
        List<Map<String, Object>> beds = castBeds(layout.get("beds"));
        Map<Long, Map<String, Object>> byId = new HashMap<>();
        for (Map<String, Object> bed : beds) {
            byId.put(number(bed.get("id")), bed);
        }
        Map<Long, BedOccupancyQueryService.BedOccupancy> occupancy =
                occupancyQueryService.describeRoom(roomId);
        ...
        for (RoomLayoutService.LayoutItem item : command.beds()) {
            ...
            for (Map<String, Object> affected : beds) {
                ...
                BedOccupancyQueryService.BedOccupancy state =
                        occupancy.get(number(affected.get("id")));
    ...
    private List<Map<String, Object>> castBeds(Object value) {
