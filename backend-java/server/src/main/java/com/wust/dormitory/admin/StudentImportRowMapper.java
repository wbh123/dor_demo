...
public class StudentImportRowMapper {
    ...
    public StudentAdminService.StudentCommand map(Map<String, String> row) {
        ...
        String nationalityText = defaultValue(
                value(row, "国家/地区", "countryregion"),
                value(row, "国籍", "nationalitycode"));
        ...
        return new StudentAdminService.StudentCommand(
                ...
                category,
                "BATCH_IMPORT",
                blankToNull(value(row, "手机号", "phonenumber")),
                degreeLevel(degree),
                gradeYear);
    }
    ...
    private String value(Map<String, String> row, String chinese, String english) {
        ...
        return row.getOrDefault(normalizedChinese, row.getOrDefault(english, "")).trim();
    }
