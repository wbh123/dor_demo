...
学生表不保存账号、组织、班级、校区、年级、专业名称快照、全局住宿资格、资料状态和数据来源。
...
同一房间中床位编码和`position_index`唯一。`position_index`继续作为无自定义布局时的默认回退依据。
...
- 一个床位最多一条布局记录；
- 一次保存必须提交房间内全部床位；
- 同一上下铺床架的上下层必须共享`layout_x`、`layout_z`和`rotation_degrees`；
- 上下层高度由`bed_type`决定，不保存可编辑高度；
- 管理端以`room.version`执行乐观锁；
- 保存后递增`room.version`和`room.state_version`；
- 修改原因和前后布局写入`audit_log`，操作类型为`ROOM_LAYOUT_UPDATE`。

## 4. 问卷三态吸烟偏好

V4将`SMOKING_ACCEPTANCE`改为单选题：
...
只有`ACCEPT`与`REJECT`直接组合时产生明显冲突，`ANY`不产生冲突。
...
测试学号范围为`202600000001`至`202600000520`。开发测试数据通过本地重建脚本的MySQL客户端导入，不写入Flyway历史。
...
V5新增`room_bed_layout`及坐标、朝向和操作者外键约束，不修改第一阶段`bed`表。
