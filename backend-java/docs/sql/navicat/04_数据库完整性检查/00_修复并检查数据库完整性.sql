...
--
-- 说明：
-- 1. 先幂等恢复运行必需的欢迎语与默认规则模板；
-- 2. 清理配置表中已经失效的管理员引用；
-- 3. 检查关键表、字段、索引、外键、初始化数据与孤儿记录；
-- 4. 任一检查失败会使用 SIGNAL 中止，全部通过输出 DB_INTEGRITY_OK。
-- ============================================================================

USE `wust_dormitory`;
SET NAMES utf8mb4;

-- 与 Flyway V17 相同的幂等修复，便于仅使用 Navicat 的环境直接恢复。
...
SELECT
    ...
    300, 1, 1, 2, 5, 1,
    'ADMIN_ALLOCATION', 'phase2-rule-template-v1',
    1,
    CASE WHEN EXISTS (SELECT 1 FROM batch_rule_template WHERE is_default=1) THEN 0 ELSE 1 END,
    NULL,
    'Navicat完整性检查恢复缺失的系统默认规则模板'
WHERE NOT EXISTS (
...
BEGIN
    ...
    CREATE TEMPORARY TABLE required_integrity_table (
    ...
    ('bed_assignment'),('assignment_history'),('allocation_run'),
    ('allocation_run_result'),('room_assignment'),('room_assignment_history'),
    ('system_setting'),('audit_log'),('feature_catalog'),('quota_catalog'),
    ...
    ('room_assignment','active_student_marker'),
    ('room_assignment','active_bed_marker'),('room_assignment','ended_at'),
    ('system_setting','setting_key'),('system_setting','setting_value'),
    ('system_setting','version'),('system_setting','updated_by');
    ...
    ('app_user','uk_single_system_admin'),
    ('system_setting','uk_system_setting_key'),
    ('active_batch_student_lock','PRIMARY'),
    ('active_batch_room_lock','PRIMARY'),
    ...
    CREATE TEMPORARY TABLE required_integrity_fk (
    ...
    ('fk_room_assignment_student'),('fk_room_assignment_room'),
    ('fk_room_assignment_bed'),('fk_subscription_revision_subscription'),
    ('fk_subscription_revision_plan');
    ...
    IF issue_count <> 1 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT='SYSTEM_ADMIN数量必须恰好为1';
    END IF;
    ...
    IF issue_count <> 1 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT='学生欢迎语配置必须恰好为1条';
    END IF;
    ...
    WHERE student_record.id IS NULL
       OR room_record.id IS NULL
       OR (residency.bed_id IS NOT NULL AND (bed_record.id IS NULL OR bed_record.room_id<>residency.room_id));
    IF issue_count > 0 THEN
...
SELECT DATABASE() AS current_database,
       'V17' AS expected_schema_version,
