...
<template>
  <div class="content-column narrow">
    ...
    <template v-if="!loading && result.assigned">
      <section class="panel assignment-result"><div class="result-check">✓</div><span class="eyebrow">ASSIGNED</span><h2>{{ assignment.building_name }}</h2><div class="assignment-address"><strong>{{ assignment.room_number }} 室</strong><span>{{ bedConfirmed ? `${assignment.bed_code} 床位` : '具体床位等待核查' }}</span></div><dl class="meta-grid"><div><dt>楼层</dt><dd>{{ assignment.floor_number }} 层</dd></div><div><dt>床位类型</dt><dd>{{ bedConfirmed ? bedTypeLabel(assignment.bed_type) : '未核查' }}</dd></div><div><dt>分配方式</dt><dd>{{ methodText(assignment.assignment_method) }}</dd></div></dl><div class="button-row"><button v-if="canReselect" class="button secondary" :disabled="cancelling" @click="requestCancelAssignment">{{ cancelling?'正在取消…':'取消并重新选择' }}</button><RouterLink class="button primary" to="/student">返回选寝首页</RouterLink></div></section>
