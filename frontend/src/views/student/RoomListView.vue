...
async function load() {
  ...
  try {
    const response = await api.get<ListSuccessResponse>(`/api/v1/student/batches/${batchId}/rooms`)
...
function recommendationReasons(room: DataObject) { return ((room.recommendationReasons ?? room.matches ?? []) as string[]).slice(0,3) }
function conflictReasons(room: DataObject) { return ((room.conflictReasons ?? room.warnings ?? []) as string[]).slice(0,2) }
</script>

<template>
  <div class="content-column">
    <div class="page-title split-title">
      <div><span class="eyebrow">{{ subtitle('宿舍匹配', 'ROOM MATCHING') }}</span><h2>{{ isTeamMode ? `为${memberCount}人队伍${isRoomMode?'选择寝室':'选择床位'}` : isRoomMode ? '选择寝室' : '选择宿舍床位' }}</h2><p v-if="isRoomMode">选择成功后只确定寝室归属，不显示或分配具体床位；入住成员自行协商实际床位。</p><p v-else>房间按偏好接近程度排序，进入房间后选择真实可用床位。</p></div>
    ...
    <div v-else class="room-grid compact-room-grid">
      <article v-for="room in filteredRooms" :key="String(room.id)" class="panel room-card">
        <div class="room-card-head"><div><span class="eyebrow">{{ room.building_name }}</span><h3>{{ room.room_number }}室</h3></div><span class="score-ring">{{ Number(room.matchScore).toFixed(0) }}</span></div>
