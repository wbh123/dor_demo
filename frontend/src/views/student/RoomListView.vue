...
const buildingOptions = computed(() => [...new Set(
  rooms.value.map(room => String(room.building_name ?? '').trim()).filter(Boolean),
)].sort((left, right) => left.localeCompare(right, 'zh-CN', { numeric: true })))
const floorOptions = computed(() => [...new Set(
  rooms.value
    .filter(room => !buildingFilter.value || String(room.building_name ?? '') === buildingFilter.value)
    .map(room => Number(room.floor_number)),
)].filter(Number.isFinite).sort((a,b)=>a-b))
const minimumAvailableBedOptions = computed(() => {
  const maximum = Math.max(0, ...rooms.value.map(room => Number(room.availableCount ?? 0)).filter(Number.isFinite))
  return Array.from({ length: maximum }, (_, index) => index + 1)
})
const roomNumberSuggestions = computed(() => {
  const term = roomKeyword.value.trim().toLowerCase()
  return [...new Set(
    rooms.value
      .filter(room => !buildingFilter.value || String(room.building_name ?? '') === buildingFilter.value)
      .filter(room => !floorFilter.value || Number(room.floor_number) === Number(floorFilter.value))
      .map(room => String(room.room_number ?? '').trim())
      .filter(Boolean),
  )]
    .filter(roomNumber => !term || roomNumber.toLowerCase().includes(term))
    .sort((left, right) => left.localeCompare(right, 'zh-CN', { numeric: true }))
    .slice(0, 50)
})
const sortedFilteredRooms = computed(() => {
  ...
  return rooms.value.filter(room => {
    ...
    return !term || String(room.room_number ?? '').toLowerCase().includes(term)
...
watch(buildingFilter, () => {
  if (floorFilter.value && !floorOptions.value.includes(Number(floorFilter.value))) floorFilter.value = ''
})
...
<template>
  <div class="content-column room-list-page">
    <div class="page-title split-title">
      <div>
        <span class="eyebrow">{{ subtitle('选择寝室','ROOM SELECTION') }}</span>
        <h2>{{ isTeamMode?'组队选择寝室':'选择寝室' }}</h2>
        <p>匹配度根据当前学生或已确认队友的偏好计算，仅用于排序参考，不代表所有生活习惯完全一致。</p>
      </div>
      <button class="button ghost" @click="router.back()">返回</button>
    </div>
    <p v-if="error" class="alert error">{{ error }}</p>
    <p v-if="message" class="alert success">{{ message }}</p>

    <section v-if="recommendationEnabled" class="panel recommendation-panel">
      <div><span class="eyebrow">智能推荐</span><h3>快速找到更合适的寝室</h3><p>可按最高匹配度、完全随机或按匹配度加权随机推荐。</p></div>
      <div class="recommendation-actions">
        <select v-model="recommendationStrategy" class="input"><option v-for="option in recommendationOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select>
        <button class="button primary" :disabled="recommending" @click="withPreferenceCheck(requestRecommendation)">{{ recommending?'推荐中…':recommendationButtonLabel }}</button>
      </div>
      <article v-if="recommendationResult&&recommendationRoom()" class="recommendation-result">
        <div><strong>{{ recommendationRoom()?.building_name }} {{ recommendationRoom()?.room_number }}室</strong><span>匹配度 {{ scoreText(recommendationRoom()!) }}分 · 可用{{ recommendationRoom()?.availableCount }}床</span></div>
        <button class="button secondary" @click="enterRoom(recommendationRoom()!)">查看并选择</button>
      </article>
    </section>
    ...
    <section class="panel room-filter-panel">
      <div class="room-filter-grid">
        <label>
          <span>楼栋</span>
          <select v-model="buildingFilter" class="input">
        ...
        <label>
          ...
          <select v-model="floorFilter" class="input">
            ...
            <option v-for="floor in floorOptions" :key="floor" :value="String(floor)">{{ floor }}层</option>
          </select>
        </label>
        <label>
          <span>寝室号</span>
          <input v-model.trim="roomKeyword" class="input" type="search" list="room-number-options" placeholder="输入寝室号关键词" autocomplete="off" />
          <datalist id="room-number-options">
            <option v-for="roomNumber in roomNumberSuggestions" :key="roomNumber" :value="roomNumber" />
          </datalist>
        </label>
        <label>
          <span>最低剩余床位</span>
          <select v-model.number="minimumAvailableBeds" class="input">
            ...
            <option v-for="count in minimumAvailableBedOptions" :key="count" :value="count">至少 {{ count }} 床</option>
          </select>
        </label>
      </div>
    </section>
    ...
    <section v-else class="room-card-grid">
      <article v-for="(room,index) in pagedRooms" :key="String(room.id)" class="panel room-selection-card" :class="{ 'room-card-compact': isRoomMode }">
        <header class="room-card-header">
          <div><span class="eyebrow">{{ room.building_name }} · {{ room.floor_number }}层</span><h3>{{ room.room_number }}室</h3><small v-if="rankLabel(index)">{{ rankLabel(index) }}</small></div>
          <div class="score-ring-with-label"><strong>{{ scoreText(room) }}</strong><span>匹配度</span></div>
        </header>
        <div class="room-fact-row">
          <span>可用 {{ room.availableCount }} 床</span>
          <span v-if="isTeamMode">队伍需 {{ memberCount }} 床</span>
        </div>
      ...
      <p v-if="!pagedRooms.length" class="panel empty-state">没有符合当前筛选条件的寝室。</p>
    </section>

    <div v-if="sortedFilteredRooms.length" class="room-pagination">
      <button class="button ghost small" :disabled="roomPage<=1" @click="roomPage--">上一页</button>
      <span>第 {{ roomPage }}/{{ totalRoomPages }} 页 · 显示 {{ currentRoomStart }}-{{ currentRoomEnd }} / {{ sortedFilteredRooms.length }}</span>
      <button class="button ghost small" :disabled="roomPage>=totalRoomPages" @click="roomPage++">下一页</button>
    </div>

    <div v-if="roomSelectionTarget" class="modal-overlay room-selection-overlay" @click.self="closeRoomSelectionConfirm">
      <section class="modal-card room-selection-dialog">
        <span class="eyebrow">确认选择寝室</span>
        <h3>{{ roomSelectionTarget.building_name }} {{ roomSelectionTarget.room_number }}室</h3>
        <p>当前批次采用寝室选择模式，确认后具体床位待寝室成员协商或由管理员后续确认。</p>
        <div class="button-row">
          <button class="button ghost" :disabled="selectingRoomId!==null" @click="closeRoomSelectionConfirm">取消</button>
          <button class="button primary" :disabled="selectingRoomId!==null" @click="confirmRoomSelection">{{ selectingRoomId!==null?'正在确认…':'确认选择此寝室' }}</button>
        </div>
      </section>
    </div>

    <AppModal :open="Boolean(selectionSuccess)" title="寝室选择成功" size="default" @close="selectionSuccess=null">
      <div v-if="selectionSuccess" class="selection-success-content"><strong>{{ selectionSuccess.buildingName }} {{ selectionSuccess.roomNumber }}室</strong><p>寝室已确定，具体床位待寝室成员协商或由管理员确认。</p></div>
      <template #footer><button class="button primary" @click="router.push('/student')">返回学生首页</button></template>
    </AppModal>
    <AppModal :open="preferencePromptVisible" title="个人偏好尚未完成" size="default" @close="preferencePromptVisible=false">
      <p>填写个人偏好后，系统才能提供更可靠的匹配度与推荐结果。</p>
      <template #footer><button class="button ghost" @click="preferencePromptVisible=false">取消</button><button class="button secondary" @click="router.push('/student/preferences')">先填写偏好</button><button v-if="selectionReadiness.allowWithoutQuestionnaire" class="button primary" @click="continueWithoutPreference">仍然继续</button></template>
    </AppModal>
...
<style scoped>
.room-filter-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.room-filter-grid label{display:grid;align-content:start;gap:5px}.recommendation-panel{display:flex;align-items:center;justify-content:space-between;gap:18px}.recommendation-panel h3,.recommendation-panel p{margin:4px 0}.recommendation-actions{display:flex;gap:8px}.recommendation-result{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px;border-radius:12px;background:var(--surface-soft)}.recommendation-result div{display:grid;gap:3px}.room-card-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}.room-selection-card{display:grid;align-content:start;gap:12px}.room-selection-card>.button{margin-top:auto}.room-card-compact{padding:16px;gap:10px}.room-card-compact .score-ring-with-label{width:60px;height:60px}.room-card-header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.room-card-header h3{margin:4px 0}.score-ring-with-label{display:grid;place-items:center;width:68px;height:68px;border:5px solid color-mix(in srgb,var(--primary) 62%,#dce5f1);border-radius:50%;background:var(--surface)}.score-ring-with-label strong{font-size:20px;line-height:1}.score-ring-with-label span{font-size:10px;color:var(--text-muted)}.room-fact-row{display:flex;gap:6px;flex-wrap:wrap}.room-fact-row span,.roommate-preference-list span{padding:4px 7px;border-radius:999px;background:var(--surface-soft);font-size:11px}.room-warning{margin:0;padding:9px;border-radius:10px;background:#fff7ed;color:#9a4c0f;font-size:12px}.conflict-reason-list{display:grid;gap:4px;margin:0;padding-left:18px;color:#9a4c0f;font-size:12px}.roommate-preference-list{display:flex;gap:5px;flex-wrap:wrap}.room-pagination{display:flex;align-items:center;justify-content:center;gap:12px}.room-selection-dialog{width:min(520px,calc(100vw - 32px));padding:24px}.room-selection-dialog .button-row{justify-content:flex-end;margin-top:18px}.selection-success-content{display:grid;gap:8px}.selection-success-content strong{font-size:22px}@media(max-width:980px){.room-filter-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.room-filter-grid{grid-template-columns:1fr}.recommendation-panel{align-items:stretch;flex-direction:column}.recommendation-actions{display:grid;grid-template-columns:1fr}.room-card-grid{grid-template-columns:1fr}}
</style>
