...
import type { EntityOption } from '../../components/common/entitySelectTypes'
...
const exchangeQuery = reactive({
  studentNumber: '',
  studentName: '',
  buildingId: null as string | number | null,
  roomNumber: '',
})
...
const exchangeModeText = computed(() => ({
  DISABLED: '未开放', ENABLED: '已开放', MUTUAL_CONFIRMATION: '双方确认后直接交换',
...
const filteredCandidates = computed(() => {
  ...
  return candidates.value
    ...
    .sort((a, b) => `${a.building_name}${a.room_number}`.localeCompare(
      `${b.building_name}${b.room_number}`, 'zh-CN', { numeric: true },
    ))
})
const changeRoomPageSize = computed(() => CHANGE_ROOM_ROWS_PER_PAGE)
const totalChangeRoomPages = computed(() => Math.max(
  1,
  Math.ceil(filteredCandidates.value.length / changeRoomPageSize.value),
))
...
const changeRoomStart = computed(() => filteredCandidates.value.length === 0
  ? 0
  : (changeRoomPage.value - 1) * changeRoomPageSize.value + 1)
const changeRoomEnd = computed(() => Math.min(
  changeRoomPage.value * changeRoomPageSize.value,
...
async function load() {
  ...
  try {
    const [profileResponse, policyResponse, historyResponse, exchangeHistoryResponse, buildingResponse] = await Promise.all([
      ...
      api.get<ListSuccessResponse>('/api/v1/student/room-exchanges/candidate-buildings'),
    ])
    ...
    buildingOptions.value = ((buildingResponse.data.data ?? []) as DataObject[]).map(item => ({
...
async function searchExchangeCandidates() {
  if (!/^\d{12}$/.test(exchangeQuery.studentNumber.trim())
      || !exchangeQuery.studentName.trim()
      || !exchangeQuery.buildingId
      || !exchangeQuery.roomNumber.trim()) {
    error.value = '请完整填写12位学号、姓名、楼栋和寝室号。'
    return
  }
  ...
  try {
    const response = await api.get<ObjectSuccessResponse>(
      '/api/v1/student/room-exchanges/exact-candidate',
      {
        params: {
          studentNumber: exchangeQuery.studentNumber.trim(),
          studentName: exchangeQuery.studentName.trim(),
          buildingId: Number(exchangeQuery.buildingId),
          roomNumber: exchangeQuery.roomNumber.trim(),
        },
      },
    )
...
async function respondExchange(item: DataObject, accepted: boolean) {
  const reason = window.prompt(
    accepted ? '请填写双方已达成一致的说明' : '请填写拒绝原因',
    accepted ? '双方已线下沟通并确认交换' : '暂不同意本次交换',
  )
  if (!reason?.trim()) return
  submitting.value = true
  try {
    const response = await api.post<ObjectSuccessResponse>(
      `/api/v1/student/room-exchanges/${item.id}/respond`,
      { accepted, reason: reason.trim() },
    )
    const result = (response.data.data ?? {}) as DataObject
    message.value = !accepted
      ? '已拒绝本次交换邀请。'
      : String(result.request_status) === 'EXECUTED'
        ? '双方寝室床位已完成交换。'
        : '已接受交换，申请已转交管理员审批。'
    await load()
  } catch (cause) {
...
<template>
  <div class="content-column room-exchange-page">
    <header class="page-title split-title">
      <div><span class="eyebrow">{{ subtitle('住宿调整', 'ROOM EXCHANGE') }}</span><h2>寝室交换</h2><p>双方需自主联系并在线下商议，达成一致后再通过系统发起和确认操作。</p></div>
    ...
    <div class="exchange-strategy-grid">
      <button class="exchange-strategy-card" :class="{ active: activeStrategy === 'CHANGE' }" type="button" @click="activeStrategy = 'CHANGE'"><span>策略一</span><strong>空余资源换寝</strong><small>{{ modeText }} · 迁入有空位的寝室</small></button>
      <button class="exchange-strategy-card" :class="{ active: activeStrategy === 'EXCHANGE' }" type="button" @click="activeStrategy = 'EXCHANGE'"><span>策略二</span><strong>双方寝室交换</strong><small>{{ exchangeModeText }} · 需双方线下沟通</small></button>
    </div>

    <template v-if="activeStrategy === 'CHANGE'">
      <section v-if="policy.enabled" class="panel">
        <div class="section-head"><div><span class="eyebrow">空余资源换寝</span><h3>符合条件的可用寝室</h3></div><input v-model="candidateKeyword" class="input compact-search" placeholder="输入楼栋或寝室号" /></div>
        <p v-if="loading" class="empty-state">正在加载…</p>
        <div v-else-if="filteredCandidates.length" class="change-room-grid">
          <article v-for="room in pagedCandidates" :key="String(room.id)" class="change-room-card"><div><span class="eyebrow">{{ room.building_name }}</span><h3>{{ room.room_number }}室</h3><p>{{ room.floor_number }}层 · 剩余{{ room.available_count }}个名额</p></div><button class="button primary" @click="requestChange(room)">选择此寝室</button></article>
        </div>
        <p v-else class="empty-state">当前没有符合条件的空余寝室。</p>
        <nav v-if="filteredCandidates.length" class="change-room-pagination" aria-label="换寝候选分页">
          <span>显示 {{ changeRoomStart }}-{{ changeRoomEnd }} / {{ filteredCandidates.length }} · 第 {{ changeRoomPage }} / {{ totalChangeRoomPages }} 页</span>
          <div class="button-row"><button class="button ghost small" :disabled="changeRoomPage <= 1" @click="changeRoomPage--">上一页</button><button class="button ghost small" :disabled="changeRoomPage >= totalChangeRoomPages" @click="changeRoomPage++">下一页</button></div>
        </nav>
      </section>
      <section v-else class="panel empty-state"><h3>学校当前未开放空余资源换寝</h3></section>
    ...
    <template v-else>
      <section v-if="exchangePolicy.enabled" class="panel">
        <div class="section-head"><div><span class="eyebrow">隐私保护精确查询</span><h3>通过双方已知信息查找交换对象</h3><p>必须同时精确匹配学号、姓名、楼栋和寝室号。系统不会列出或模糊搜索其他学生。</p></div></div>
        <form class="exact-exchange-form" @submit.prevent="searchExchangeCandidates">
          <label><span>12位学号</span><input v-model.trim="exchangeQuery.studentNumber" class="input" maxlength="12" inputmode="numeric" required /></label>
          <label><span>姓名</span><input v-model.trim="exchangeQuery.studentName" class="input" maxlength="128" required /></label>
          <label><span>楼栋</span><RemoteEntitySelect v-model="exchangeQuery.buildingId" :options="buildingOptions" placeholder="请选择楼栋" search-placeholder="搜索楼栋" /></label>
          <label><span>寝室号</span><input v-model.trim="exchangeQuery.roomNumber" class="input" maxlength="32" required /></label>
          <button class="button secondary" :disabled="exchangeSearching">{{ exchangeSearching ? '正在精确匹配…' : '精确查询' }}</button>
        </form>
