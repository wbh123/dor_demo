...
const validPassword = computed(() => newPassword.value.length > 0 && newPassword.value.length <= 72)

async function submit() {
  error.value = ''
  if (!validPassword.value) { error.value = '开发阶段仅要求新密码非空且不超过72位。'; return }
...
<template>
  <div class="content-column narrow">
    <div class="page-title"><span class="eyebrow">ACCOUNT SECURITY</span><h2>修改管理员密码</h2><p>开发阶段暂不校验密码复杂度；修改成功后全部登录令牌立即失效。</p></div>
    <section class="panel password-card"><form class="form-stack" @submit.prevent="submit"><label><span>当前密码</span><input v-model="currentPassword" class="input" type="password" autocomplete="current-password" required /></label><label><span>新密码</span><input v-model="newPassword" class="input" type="password" autocomplete="new-password" required maxlength="72" /></label><div class="password-rules"><strong>开发阶段密码要求</strong><span :class="{ pass: validPassword }">非空且不超过72位</span></div><label><span>确认新密码</span><input v-model="confirmPassword" class="input" type="password" autocomplete="new-password" required maxlength="72" /></label><p v-if="error" class="alert error">{{ error }}</p><button class="button primary" :disabled="saving">{{ saving ? '正在修改…' : '修改密码并重新登录' }}</button></form></section>
  </div>
</template>
