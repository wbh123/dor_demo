...
const validPassword = computed(() => newPassword.value.length >= 4 && newPassword.value.length <= 72)

async function submit() {
  error.value = ''
  if (!validPassword.value) { error.value = '新密码暂时要求为4至72位。'; return }
...
<template>
  <div class="content-column narrow">
    ...
    <section class="panel password-card"><form class="form-stack" @submit.prevent="submit"><label><span>当前密码</span><input v-model="currentPassword" class="input" type="password" autocomplete="current-password" required /></label><label><span>新密码</span><input v-model="newPassword" class="input" type="password" autocomplete="new-password" minlength="4" maxlength="72" required /></label><div class="password-rules"><strong>临时密码要求</strong><span :class="{ pass: validPassword }">4至72位</span><small>当前处于测试阶段，暂不强制大小写、数字和特殊字符组合。</small></div><label><span>确认新密码</span><input v-model="confirmPassword" class="input" type="password" autocomplete="new-password" minlength="4" maxlength="72" required /></label><p v-if="error" class="alert error">{{ error }}</p><button class="button primary" :disabled="saving">{{ saving ? '正在修改…' : '修改密码并重新登录' }}</button></form></section>
  </div>
</template>

<style scoped>.password-card{padding:24px}.password-rules{display:flex;gap:8px;flex-wrap:wrap;padding:12px;border-radius:12px;background:var(--soft)}.password-rules strong{width:100%}.password-rules span{padding:4px 8px;border-radius:999px;color:#9b2838;background:#fff0f2;font-size:12px}.password-rules span.pass{color:#17664f;background:#e8f8f2}.password-rules small{width:100%;color:var(--muted)}</style>
