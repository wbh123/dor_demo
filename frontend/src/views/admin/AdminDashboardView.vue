...
function setEditorRef(locale: string, value: unknown) {
  if (value) editorRefs.set(locale, value as WelcomeEditorExpose)
  else editorRefs.delete(locale)
}
...
function removeWelcomeLanguage(locale: string) {
  ...
  if (activeLocale.value === locale) activeLocale.value = 'zh-CN'
}
...
function insertPlaceholder(name: string) {
  const editor = editorRefs.get(activeLocale.value) ?? editorRefs.get('zh-CN')
...
async function saveWelcomeSetting() {
  ...
  try {
    ...
    welcomeUpdatedAt.value = String(data.updated_at ?? '')
    welcomeUpdatedBy.value = String(data.updated_by_name ?? '')
    showToast('新生欢迎语已保存。未配置的外文语言将展示英文版本。')
  } catch (reason) {
    welcomeError.value = translateError(reason)
  } finally {
    ...
    <template v-else>
      ...
      <section class="panel welcome-setting-card">
        <div class="section-head split-title compact-section-head">
          <div><span class="eyebrow">{{ subtitle('首次登录欢迎', 'FIRST LOGIN WELCOME') }}</span><h3>新生欢迎语语言版本</h3><p>中文和英文为基础版本；其他语言未配置时统一展示英文欢迎语。</p></div>
        </div>
        ...
        <div class="language-editor-grid">
          <article v-for="localeCode in orderedLocales" :key="localeCode" class="language-editor-card">
            ...
            <WelcomeMessageEditor
              :ref="(value) => setEditorRef(localeCode, value)"
              v-model="languageMessages[localeCode]"
              :token-examples="tokenExamples"
              :placeholder="`请输入 ${localeLabel(localeCode)} 欢迎语`"
              @focus="activeLocale = localeCode"
            />
            <footer>
              <small>{{ languageMessages[localeCode]?.length ?? 0 }}/1000</small>
              <button v-if="localeCode === 'en-US'" class="button secondary small translate-button" type="button" :disabled="translating" @click="translateChinese">{{ translating ? '正在翻译…' : '重新翻译' }}</button>
            </footer>
          </article>
        </div>
        ...
        <div class="placeholder-toolbar">
          ...
          <small>当前插入到：{{ localeLabel(activeLocale) }}</small>
        </div>

        <div class="language-add-row">
          <label><span>按语言或国家地区添加欢迎语</span><select v-model="newLocale" class="input"><option v-for="option in welcomeLanguageOptions" :key="option.value" :value="option.value">{{ option.label }} · {{ option.value }}</option></select></label>
      ...
      <div class="admin-grid">
        <section class="panel span-2">
          <div class="section-head compact-section-head"><div><span class="eyebrow">{{ subtitle('选寝批次', 'BATCHES') }}</span><h3>最近选寝批次</h3></div><RouterLink class="button ghost" to="/admin/batches">管理批次</RouterLink></div>
          <div class="table-wrap"><table><thead><tr><th>批次</th><th>状态</th><th>资格人数</th><th>已分配</th></tr></thead><tbody><tr v-for="batch in batches.slice(0, 6)" :key="String(batch.id)"><td><strong>{{ batch.batch_name }}</strong><small>{{ batch.batch_code }}</small></td><td><span class="status-chip compact">{{ batchStatus(batch.batch_status) }}</span></td><td>{{ batch.eligible_count }}</td><td>{{ batch.assigned_count }}</td></tr></tbody></table></div>
        </section>

        <section class="panel">
          <div class="section-head compact-section-head"><div><span class="eyebrow">{{ subtitle('操作记录', 'ACTIVITY') }}</span><h3>最近操作</h3></div></div>
