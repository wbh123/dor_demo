...
async function translateChinese() {
  ...
  try {
    ...
    welcomeError.value = '自动翻译暂时不可用，请直接编辑英文欢迎语。'
  } finally {
...
async function saveWelcomeSetting() {
  ...
  const countries = Object.fromEntries(Object.entries(countryMessages.value)
    .map(([code, message]) => [code, message.trim()])
  ...
  <div class="content-column">
    ...
    <template v-else>
      ...
      <section class="panel welcome-setting-card">
        ...
        <div class="additional-country-section">
          <div><h4>其他国家或地区</h4><p>选择已配置国家进行修改，或从尚未配置的国家列表中添加。</p></div>
          <CountryWelcomeEditor v-model="countryMessages" v-model:selected-country="selectedCountry" :token-examples="tokenExamples" />
        </div>
        ...
        <div class="button-row welcome-actions"><button class="button primary" :disabled="welcomeSaving" @click="saveWelcomeSetting">{{ welcomeSaving ? '正在保存…' : '保存全部欢迎语' }}</button></div>
      </section>

      <div class="dashboard-lower-grid">
        <section class="panel"><div class="section-head"><div><span class="eyebrow">选寝批次</span><h3>最近批次</h3></div></div><div class="compact-list"><article v-for="batch in batches.slice(0,6)" :key="String(batch.id)"><strong>{{ batch.batch_name }}</strong><span>{{ batchStatus(batch.batch_status) }}</span></article><p v-if="!batches.length" class="empty-state">暂无选寝批次。</p></div></section>
        <section class="panel"><div class="section-head"><div><span class="eyebrow">最近操作</span><h3>业务操作记录</h3></div></div><div class="compact-list"><article v-for="log in auditLogs" :key="String(log.id)"><strong>{{ log.operator_name || '系统' }}</strong><span>{{ log.action || log.operation_type || '完成业务操作' }}</span></article><p v-if="!auditLogs.length" class="empty-state">暂无操作记录。</p></div></section>
      </div>
    </template>
  </div>
</template>
...
<style scoped>
.stat-value-line{display:flex;align-items:baseline;justify-content:space-between;gap:12px}.stat-value-line small{color:var(--muted)}.base-country-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.base-country-card{display:grid;gap:12px;padding:16px;border:1px solid var(--line);border-radius:16px;background:var(--soft)}.base-country-card header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.base-country-card header div{display:grid;gap:4px}.base-country-card small,.additional-country-section p,.welcome-meta{color:var(--muted)}.required-chip{padding:4px 8px;border-radius:999px;color:#17664f;background:#e8f8f2;font-size:12px}.additional-country-section{display:grid;gap:14px;margin-top:22px;padding-top:20px;border-top:1px solid var(--line)}.additional-country-section h4,.additional-country-section p{margin:0}.additional-country-section p{margin-top:5px}.welcome-meta{display:flex;gap:18px;flex-wrap:wrap;margin-top:14px;font-size:12px}.welcome-actions{justify-content:flex-end;margin-top:14px}.dashboard-lower-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.compact-list{display:grid;gap:8px}.compact-list article{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:11px;border-radius:12px;background:var(--soft)}.compact-list span{color:var(--muted)}@media(max-width:760px){.base-country-grid,.dashboard-lower-grid{grid-template-columns:1fr}}
</style>
