...
<template>
  <div class="content-column mobile-room-review">
    ...
    <div v-else class="review-room-grid"><article v-for="room in rooms" :key="String(room.room_id)" class="panel review-room-card" @click="openRoom(room)"><header><div><strong>{{ room.building_name }} {{ room.room_number }}室</strong><span>{{ room.floor_number }}层 · {{ room.resident_count }}人</span></div><b>{{ room.pending_count }} 条待核查</b></header><div class="room-review-counts"><span>冲突 {{ room.conflict_count??0 }}</span><span>容量 {{ room.capacity }}</span></div><button class="button secondary full">进入寝室核查</button></article><p v-if="!rooms.length" class="panel empty-state">当前没有待核查的寝室。</p></div>
