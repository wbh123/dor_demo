...
function methodText(value: unknown) {
  return ({ SELF_SELECT: '个人自选', TEAM_SELECT: '队伍自选', STUDENT_RANDOM: '学生随机', ADMIN_RANDOM: '统一分配', MANUAL_ADJUSTMENT: '人工调整' } as Record<string, string>)[String(value)] ?? String(value)
}
...
<template>
  <div class="content-column">
    ...
    <section v-if="selectedAssignment" class="panel drawer-panel">
      <div class="section-head compact-section-head"><div><span class="eyebrow">人工调整</span><h3>调整 {{ selectedAssignment.student_name }} 的床位</h3><p>当前：{{ selectedAssignment.building_name }} {{ selectedAssignment.room_number }}-{{ selectedAssignment.bed_code }}</p></div></div>
      <form class="form-grid two-column" @submit.prevent="submitAdjustment"><label><span>目标床位</span><select v-model.number="adjustment.newBedId" class="input" required><option v-for="bed in targetBeds" :key="String(bed.bed_id)" :value="Number(bed.bed_id)">{{ bed.display_name }} · {{ bedTypeLabel(bed.bed_type) }}</option></select></label><label><span>调整原因</span><input v-model.trim="adjustment.reason" class="input" required minlength="2" maxlength="500" placeholder="例如：原床位进入维护状态" /></label><div class="button-row span-2"><button type="button" class="button ghost" @click="selectedAssignment = null">取消</button><button class="button primary">确认调整</button></div></form>
    </section>
  </div>
</template>

<style scoped>
.compact-section-head{margin-bottom:12px}
</style>
