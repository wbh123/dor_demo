...
<template><section class="panel"><h1>修改系统管理员密码</h1><p>调试阶段仅要求密码非空，不限制长度、字符种类、组合，也允许与当前密码相同。项目上线前将统一恢复密码强度校验。</p>
<form @submit.prevent="submit"><input v-model="currentPassword" type="password" autocomplete="current-password" placeholder="当前密码" required /><input v-model="newPassword" type="password" autocomplete="new-password" placeholder="新密码" required /><input v-model="confirmPassword" type="password" autocomplete="new-password" placeholder="确认新密码" required /><p v-if="error" class="error">{{ error }}</p><button :disabled="loading">{{ loading ? '修改中…' : '修改并重新登录' }}</button></form></section></template>
