...
<style scoped>
.app-modal-backdrop{box-sizing:border-box;position:fixed;inset:0;display:grid;place-items:center;padding:max(18px,env(safe-area-inset-top)) max(18px,env(safe-area-inset-right)) max(18px,env(safe-area-inset-bottom)) max(18px,env(safe-area-inset-left));background:rgba(15,23,42,.58);backdrop-filter:blur(3px);overflow:hidden}.app-modal-surface{box-sizing:border-box;display:flex;flex-direction:column;width:min(720px,100%);max-height:var(--app-modal-max-height);border:1px solid rgba(148,163,184,.32);border-radius:24px;background:var(--panel,#fff);color:var(--text,#172033);box-shadow:0 28px 80px rgba(15,23,42,.28);outline:none;overflow:hidden}.app-modal--compact{width:min(560px,100%)}.app-modal--wide{width:min(980px,100%)}.app-modal--large{width:min(1180px,100%);max-height:var(--app-modal-max-height)}.app-modal--fullscreen{width:100%;height:100%;max-height:none;border-radius:18px}.app-modal-header{box-sizing:border-box;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:22px 24px 18px;border-bottom:1px solid var(--line,#e5e7eb);background:var(--panel,#fff)}.app-modal-heading{min-width:0}.app-modal-heading h2{margin:0;font-size:clamp(20px,2vw,27px);line-height:1.25}.app-modal-heading p{margin:7px 0 0;color:var(--muted,#64748b);line-height:1.6}.app-modal-close{display:grid;place-items:center;flex:0 0 36px;width:36px;height:36px;border:0;border-radius:12px;background:var(--soft,#f1f5f9);font-size:24px;line-height:1;cursor:pointer}.app-modal-close:hover:not(:disabled){background:#e2e8f0}.app-modal-close:disabled{cursor:not-allowed;opacity:.5}.app-modal-body{box-sizing:border-box;position:relative;width:100%;min-width:0;flex:1;min-height:0;padding:22px 24px;overflow:auto;overscroll-behavior:contain}.app-modal-footer{box-sizing:border-box;display:flex;flex-wrap:wrap;justify-content:flex-end;align-items:center;gap:10px;padding:16px 24px 20px;border-top:1px solid var(--line,#e5e7eb);background:var(--panel,#fff)}.app-modal-loading{position:absolute;z-index:3;inset:0;display:flex;align-items:center;justify-content:center;gap:10px;background:rgba(255,255,255,.78);font-weight:700}.app-modal-spinner{width:22px;height:22px;border:3px solid #dbeafe;border-top-color:#2563eb;border-radius:50%;animation:app-modal-spin .8s linear infinite}.app-modal-fade-enter-active,.app-modal-fade-leave-active{transition:opacity .18s ease}.app-modal-fade-enter-active .app-modal-surface,.app-modal-fade-leave-active .app-modal-surface{transition:transform .18s ease,opacity .18s ease}.app-modal-fade-enter-from,.app-modal-fade-leave-to{opacity:0}.app-modal-fade-enter-from .app-modal-surface,.app-modal-fade-leave-to .app-modal-surface{transform:translateY(10px) scale(.985);opacity:0}@keyframes app-modal-spin{to{transform:rotate(360deg)}}@media(max-width:640px){.app-modal-backdrop{place-items:end center;padding:8px max(8px,env(safe-area-inset-right)) max(8px,env(safe-area-inset-bottom)) max(8px,env(safe-area-inset-left))}.app-modal-surface,.app-modal--compact,.app-modal--wide,.app-modal--large{width:100%;max-height:min(var(--app-modal-max-height),calc(100dvh - 16px));border-radius:20px}.app-modal--fullscreen{height:calc(100dvh - 16px)}.app-modal-header{padding:18px 18px 14px}.app-modal-body{padding:18px}.app-modal-footer{padding:14px 18px calc(16px + env(safe-area-inset-bottom));flex-wrap:wrap}.app-modal-footer :deep(.button){flex:1 1 140px}}
</style>
