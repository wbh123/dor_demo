...
function resetStandardLayout(){const placements=[[-2.35,-1.65],[2.35,-1.65],[-2.35,1.65],[2.35,1.65],[-.8,0],[.8,0]];units.value.forEach((unit,index)=>{const [x,z]=placements[index]??[0,Math.min(MAX_Z,-2.8+index*1.1)];updateUnit(unit.key,x,z,0)});message.value='已恢复标准2×2布局，请填写原因后保存。'}
...
<template>
  <div class="modal-overlay room-layout-overlay" @click.self="emit('close')"><section class="modal-card room-layout-dialog">
    <header class="section-head split-title"><div><span class="eyebrow">{{ subtitle('床位布局','ROOM LAYOUT') }}</span><h3>{{ roomLabel }}床位布局</h3><p>床体独立旋转，文字和操作控件始终保持正向；支持上床下桌、上下铺和单人床。</p></div><div class="button-row"><button class="button ghost" @click="resetStandardLayout">恢复标准2×2布局</button><button class="button ghost" @click="emit('close')">关闭</button></div></header>
    ...
    <template v-else>
      ...
      <div ref="stage" class="layout-stage">
        <article v-for="unit in units" :key="unit.key" class="layout-unit layout-bed-unit" :class="[unit.unitType.toLowerCase(),{occupied:unit.occupied,vertical:isQuarterTurn(unit.rotation)}]" :style="unitStyle(unit)" @pointerdown="startDrag(unit,$event)">
          ...
          <div class="layout-bed-content"><strong>{{ unit.label }}</strong><span>{{ typeLabel(unit.unitType) }}</span><div class="layout-bed-type-actions"><button type="button" @click="rotate(unit)">旋转90°</button></div></div>
        </article>
      </div>
