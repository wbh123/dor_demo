...
const units = computed<Unit[]>(() => {
  ...
  return [...groups.entries()].map(([key,items]) => {
    ...
    const reasons=[...new Set(sorted.map(item=>item.blockingReason).filter(Boolean))]
    const occupants=[...new Set(sorted.map(item=>{
      if (!item.occupantStudentName) return ''
      return item.occupantStudentNumber
        ? `${item.occupantStudentName}（${item.occupantStudentNumber}）`
        : item.occupantStudentName
    }).filter(Boolean))]
    const sources=[...new Set(sorted.map(item=>item.occupancySource).filter(value=>value&&value!=='AVAILABLE'))]
    return {
      key,
      ...
      originalType,
      unitType:selectedTypes.value[key]??originalType,
      occupied:sorted.some(item=>item.occupied),
      occupancySource:sources.join('、'),
      occupantSummary:occupants.join('、'),
...
const projectedCapacity = computed(() => {
  const expandedBunks=units.value.filter(unit=>unit.originalType!=='BUNK'&&unit.unitType==='BUNK').length
  const collapsedBunks=units.value.filter(unit=>unit.originalType==='BUNK'&&unit.unitType!=='BUNK').length
  return beds.value.length+expandedBunks-collapsedBunks
})
...
async function load(){
  ...
  try{
    const [layoutResponse,occupancyResponse]=await Promise.all([
      api.get<ObjectSuccessResponse>(`/api/v1/admin/rooms/${props.roomId}/bed-layout`),
      api.get<ListSuccessResponse>(`/api/v1/admin/rooms/${props.roomId}/bed-occupancy`),
    ])
    const data=(layoutResponse.data.data??{}) as DataObject
    const room=(data.room??{}) as DataObject
    const occupancyByBed=new Map<number,DataObject>(
      ((occupancyResponse.data.data??[]) as DataObject[]).map(item=>[Number(item.bedId),item]),
    )
    roomVersion.value=Number(room.room_version??0)
    beds.value=((data.beds??[]) as DataObject[]).map(item=>{
      const occupancy=occupancyByBed.get(Number(item.id))??{}
      return {
        id:Number(item.id),
        bed_code:String(item.bed_code),
        bed_type:String(item.bed_type),
        position_index:Number(item.position_index),
        bed_frame_id:item.bed_frame_id==null?null:Number(item.bed_frame_id),
        occupied:Boolean(occupancy.occupied??(Number(item.occupied??0)===1)),
        occupancySource:String(occupancy.occupancySource??(Number(item.occupied??0)===1?'OCCUPIED':'AVAILABLE')),
        occupantStudentName:String(occupancy.occupantStudentName??''),
        occupantStudentNumber:String(occupancy.occupantStudentNumber??''),
        blockingReason:String(occupancy.blockingReason??''),
        layout_x:Number(item.layout_x),
        layout_z:Number(item.layout_z),
...
function setType(unit:Unit,type:UnitType){
  ...
  if(unit.occupied&&type!==unit.originalType){error.value=unit.blockingReason||'床位已有学生使用或处于业务处理中，不能修改类型。';return}
...
<template>
  <div class="modal-overlay room-layout-overlay" @click.self="emit('close')"><section class="modal-card room-layout-dialog">
    ...
    <template v-else>
      ...
      <div class="capacity-summary"><strong>预计容量 {{ projectedCapacity }} / 8 人</strong><span>单人床和上床下桌各占1个床位，上下铺占2个床位。上下铺两张床均空闲时可合并为单人床或上床下桌。</span></div>
