...
const projectedCapacity = computed(() => { const expandedBunks=units.value.filter(unit=>unit.originalType!=='BUNK'&&unit.unitType==='BUNK').length; const collapsedBunks=units.value.filter(unit=>unit.originalType==='BUNK'&&unit.unitType!=='BUNK').length; return beds.value.length+expandedBunks-collapsedBunks })
