...
operations = read("frontend/src/views/admin/AdminOperationsView.vue")
require(operations, "const batches", "公平性预演必须加载可选批次")
require(operations, "<select v-model.number=\"form.batchId\"", "公平性预演批次必须使用下拉框")
require(operations, 'v-if="loading"', "运营数据加载期间必须只显示加载状态")
require(operations, "正在加载运营与健康数据", "运营页必须提供明确加载文案")
