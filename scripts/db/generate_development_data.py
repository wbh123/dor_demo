...
STUDENT_COUNT = 520
MALE_STUDENT_COUNT = 260
FEMALE_STUDENT_COUNT = 260
MAJOR_COUNT = 5
BUILDING_COUNT = 8
BUILDINGS_PER_GENDER = 4
FLOORS_PER_BUILDING = 4
MALE_ROOMS_PER_FLOOR = 4
FEMALE_ROOMS_PER_FLOOR = 5
MALE_ROOM_COUNT = BUILDINGS_PER_GENDER * FLOORS_PER_BUILDING * MALE_ROOMS_PER_FLOOR
FEMALE_ROOM_COUNT = BUILDINGS_PER_GENDER * FLOORS_PER_BUILDING * FEMALE_ROOMS_PER_FLOOR
ROOM_COUNT = MALE_ROOM_COUNT + FEMALE_ROOM_COUNT
MALE_BED_COUNT = MALE_ROOM_COUNT * 5
FEMALE_BED_COUNT = FEMALE_ROOM_COUNT * 4
BED_COUNT = MALE_BED_COUNT + FEMALE_BED_COUNT
...
def build_students() -> list[StudentSeed]:
    students: list[StudentSeed] = []
    for index in range(1, STUDENT_COUNT + 1):
        gender = "M" if index <= MALE_STUDENT_COUNT else "F"
        gender_index = index if gender == "M" else index - MALE_STUDENT_COUNT
        students.append(
            StudentSeed(
                id=index,
                student_number=f"2026{index:08d}",
                student_name=f"测试{'男' if gender == 'M' else '女'}生{gender_index:03d}",
                gender=gender,
                major_id=1 + ((index - 1) % MAJOR_COUNT),
            )
        )
    return students


def build_buildings() -> list[BuildingSeed]:
    return [
        BuildingSeed(
            id=index,
            gender="M" if index <= BUILDINGS_PER_GENDER else "F",
        )
        for index in range(1, BUILDING_COUNT + 1)
    ]


def build_floors(buildings: list[BuildingSeed]) -> list[FloorSeed]:
    floors: list[FloorSeed] = []
    floor_id = 1
    for building in buildings:
        for number in range(1, FLOORS_PER_BUILDING + 1):
            floors.append(FloorSeed(floor_id, building.id, number))
            floor_id += 1
    return floors


def build_rooms(
    buildings: list[BuildingSeed], floors: list[FloorSeed]
) -> list[RoomSeed]:
    gender_by_building = {building.id: building.gender for building in buildings}
    rooms: list[RoomSeed] = []
    room_id = 1
    for floor in floors:
        gender = gender_by_building[floor.building_id]
        rooms_per_floor = (
            MALE_ROOMS_PER_FLOOR if gender == "M" else FEMALE_ROOMS_PER_FLOOR
        )
        for _ in range(rooms_per_floor):
            rooms.append(
                RoomSeed(
                    ...
                    floor_id=floor.id,
                    gender=gender,
                    room_type="FIVE_PERSON" if gender == "M" else "FOUR_PERSON",
                    capacity=5 if gender == "M" else 4,
                )
            )
            room_id += 1
    return rooms
...
def build_beds(rooms: list[RoomSeed]) -> list[BedSeed]:
    beds: list[BedSeed] = []
    bed_id = 1
    for room in rooms:
        layout = (
        ...
        for bed_type, frame_id in layout:
            beds.append(
                BedSeed(
                    id=bed_id,
                    room_id=room.id,
...
def validate_dataset() -> None:
    ...
    floors = build_floors(buildings)
    rooms = build_rooms(buildings, floors)
    beds = build_beds(rooms)

    assert len(students) == STUDENT_COUNT
    assert len({student.student_number for student in students}) == STUDENT_COUNT
    assert all(
        len(student.student_number) == 12
        and student.student_number.isdigit()
        and student.student_number.startswith("2026")
        for student in students
    )
    assert sum(student.gender == "M" for student in students) == MALE_STUDENT_COUNT
    assert sum(student.gender == "F" for student in students) == FEMALE_STUDENT_COUNT
    assert {student.major_id for student in students} == set(range(1, MAJOR_COUNT + 1))

    assert len(buildings) == BUILDING_COUNT
    assert len(floors) == BUILDING_COUNT * FLOORS_PER_BUILDING
    assert len(rooms) == ROOM_COUNT
    assert sum(room.room_type == "FIVE_PERSON" for room in rooms) == MALE_ROOM_COUNT
    assert sum(room.room_type == "FOUR_PERSON" for room in rooms) == FEMALE_ROOM_COUNT
    assert all(room.gender in {"M", "F"} for room in rooms)

    assert len(beds) == BED_COUNT
    assert sum(bed.gender == "M" for bed in beds) == MALE_BED_COUNT
    assert sum(bed.gender == "F" for bed in beds) == FEMALE_BED_COUNT
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="生成第一阶段本地开发测试数据 Flyway SQL。"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    print(
        f"已生成 {args.output}：学生 {STUDENT_COUNT} 人，专业 {MAJOR_COUNT} 个，"
        f"床位 {BED_COUNT} 个，男生五人间 {MALE_ROOM_COUNT} 个，"
        f"女生四人间 {FEMALE_ROOM_COUNT} 个。"
    )
    return 0
